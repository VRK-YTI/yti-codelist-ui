    ...
    <dl *ngIf="editing || selectableOrganizations.length > 0">
      ...
      <dd>
        ...
        <div *ngIf="editing">
          <div *ngFor="let organization of selectableOrganizations">
            <a>
              <i [id]="'remove_' + organization.id + '_organization_link'"
                 class="fa fa-times"
                 (click)="removeOrganization(organization)"></i>
            </a>
            <span>{{organization.getDisplayName(languageService)}}</span>
          </div>
        ...
        <button id="add_organization_button"
                ...
                *ngIf="editing"
                (click)="addOrganization()" translate>Add organization
        </button>
      </dd>
    </dl>
  `
...
export class OrganizationsInputComponent implements ControlValueAccessor {
  ...
  get selectableOrganizations(): Organization[] {
    return (this.control.value as Organization[]).sort(comparingPrimitive<Organization>(organization => organization.id));
  }
